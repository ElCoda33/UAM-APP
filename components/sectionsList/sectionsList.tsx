...
import SectionsAddForm from '@/app/dashboard/sections/add/sectionsAddForm'
...
export default function SectionsList() {
    ...
    const [sectionUpdate, setSectionUpdate] = React.useState<Section>()
    const { isOpen, onOpen, onOpenChange } = useDisclosure()

    React.useEffect(() => {
        try {
            fetchSecciones().then((res: any) => {
                setSections(res)
                setIsLoading(false)
            })
        } catch (err) { }
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredUsers = filteredUsers.filter((section) =>
                section.nombre.toLowerCase().includes(filterValue.toLowerCase())
            )
        }
        if (
            statusFilter !== 'all' &&
            ...
            filteredUsers = filteredUsers.filter((section) =>
                Array.from(statusFilter).includes(section.nombre)
            )
        }
